...
class Attacker(BatchAttack):
    ...
    def _get_gradients(self, loss_type="ce"):
        ...
        if loss_type == 'ce':
            ...
            label_score = tf.reduce_sum(mask * logits-(1-mask)*1e4, axis=1)
            ...
            mask = tf.one_hot(self.ys_var, depth=tf.shape(logits)[1])
            loss = tf.reduce_sum(mask * logits-(1-mask)*1e4, axis=1)
    ...
    def batch_attack(self, xs, ys=None, ys_target=None):
        ...
        for i in range(self.iteration):
            ...
            if loss[0] < prev_loss + 1e-3 and restart_count >= 10:
                xs_adv = xs_prev
            ...
            if restart_count >= 10: prev_loss = loss[0]
